...
Each pilot is analyzed within ±350 Hz. `CorroboratedDetector` exposes motion only when both pilots report the same direction and compatible frequency-scaled shifts; the weaker confidence wins. The tone amplitudes are divided by √2 to preserve total RMS output power. A new interfering sound near only one pilot is rejected.

The timestamp-based recognizer requires consistent direction and confidence, then locks out return strokes until a cooldown and a quiet interval have both passed. Lost or stale audio requires stillness before rearming. Opposing motion invalidates accumulated evidence even if it falls below the acceptance confidence. Response presets adjust these thresholds. Practice mode shows accepted directions without emitting actions. A nonactivating panel gives immediate gesture feedback across Spaces; it confirms recognition, not whether the target app handled the command.
...
Add `--check-duration 30` to `--audio-check` for a longer check without gesture actions (7–60 seconds). Inspect `acceptedGestures` as well as signal quality: zero posted events alone does not establish idle stability when control is disabled. Synthetic regression tests include pilot fading, stationary interference, single-tone interference, conflicting directions, and compatible movements on both tones.
